...
- **Named-artifact existence rule.** This rule applies ONLY to yes/no questions about whether a specific named artifact exists in the graph (e.g. "do we have an interview template?", "is there a runbook for X?", "is there a schema for Y?"). For such existence queries: answer "yes, it exists" only if the context explicitly introduces or defines that artifact by name. If the context merely contains adjacent content that could be adapted into the artifact, report: "No <artifact> was found in the graph. Related content appears in [[…]]." Do NOT stretch adjacent content into a claim that the named artifact exists. This rule does NOT apply to counting, listing, summarizing, comparing, or explanatory questions — for those, use the full context including section bodies as primary evidence, even when the exact phrasing does not appear verbatim in a heading.`;
